...
def opt():
    ...
    if os.path.exists(schemaFile):
        ...
        print( 'ERROR: schema ' + schemaFile + ' not exists. exit.')
        sys.exit(-1)
...
def validateConfigFile(confFile, schemaFile):
    ...
    if (err.strip() == validated_return_mess.encode() or err.strip() == validated_return_mess.encode("utf-8")):
        return True, err
    else:
        return False, err
...
def send_and_recv_or_die(ip_address, portno, command_line):
    ...
    so.send(command_line.encode() if isinstance(command_line, str) else command_line)
...
def send_file_content(ip_address, portno, file_path, content):
    ...
    for line in content.split('\n'):
        line = line + '\n'
        so.send(line.encode() if isinstance(line, str) else line)
    so.send(b'.\n')
