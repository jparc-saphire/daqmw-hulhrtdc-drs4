...
void check_time(char* runno)
{
  ...
  TH2F *htmp = new TH2F("hoge", "hoge", 8000, 180, 220, 8000, 180, 220);
  ...
  for(int i = 0; i<n_event; ++i){
    ...
    for(int j = 0; j<Nhit; ++j){
      ...
      if(Ch[j] == 2) t2= s_tdc[j]*time_semi + count/NofEntry*time_semi;
      if(Ch[j] == 3) t3= s_tdc[j]*time_semi + count/NofEntry*time_semi;
    }// for(j)
