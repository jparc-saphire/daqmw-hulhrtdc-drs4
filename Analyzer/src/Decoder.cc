...
Decoder::getNextEvent()
{
  ...
  #ifdef E16DAQ
    //sako 2024/4/7
    sMZNData.MZN_event_number[imod]=eventNum;
    int spilltemp = 0;
    int eventtemp[3] = {0};
    //spill ID
    // if(mznheader3 & (1<<19)) std::cout << "!!!!!!!!" << std::endl;
    if(mznheader3 & (1<<19)) spilltemp = 1;
    
    //event ID
    for(int i = 0; i<3; ++i){
      eventtemp[i] = 0;
      if(mznheader3 & (1<<(i+16))) eventtemp[i] = 1;
    }
    
    sMZNData.spillID = spilltemp;
    sMZNData.eventID[0] = eventtemp[0];
    sMZNData.eventID[1] = eventtemp[1];
    sMZNData.eventID[2] = eventtemp[2];
    int event_number = sMZNData.eventID[0] + sMZNData.eventID[1]*2 + sMZNData.eventID[2]*4;
    //    std::cout << "event_number, sMZNData.Nhit = " << event_number << " " << sMZNData.Nhit << std::endl;
#endif
  ...
  for(int i=0;i<mznnword;i++){
    ...
    if(data_header == MZN_HRTDC::magic_leading){
      ...
      //def      sMZNData.Ch[sMZNData.Nhit]          = ((rdata >> MZN_HRTDC::shift_ch)  & MZN_HRTDC::mask_ch) + index_slot*32;
      //sako
      sMZNData.Ch[sMZNData.Nhit]          = ((rdata >> MZN_HRTDC::shift_ch)  & MZN_HRTDC::mask_ch) + index_slot*32+imod*64;
    ...
    if(data_header ==  MZN_HRTDC::magic_trailing){
      ...
      //def      sMZNData.tCh[sMZNData.tNhit]          = ((rdata >> MZN_HRTDC::shift_ch)  & MZN_HRTDC::mask_ch) + index_slot*32;
      //sako
      sMZNData.tCh[sMZNData.tNhit]          = ((rdata >> MZN_HRTDC::shift_ch)  & MZN_HRTDC::mask_ch) + index_slot*32+imod*64;
      unsigned int fc                 = (rdata)              & MZN_HRTDC::mask_fc;
