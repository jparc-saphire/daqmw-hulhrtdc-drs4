#define HIST
#define E16DAQ
#define  check_drs4_run0e_e16_cc
#include "check_drs4_run0e_e16.h"
#include <iostream>
#include <stdexcept>
#include <vector>
#include <algorithm>
...
static const Bool_t debug1=true;
static const Bool_t debug2 = false;
...
int main(int argc,char *argv[]) {
  ...
  cout << "argc = " << argc << endl;
  if (argc<=1) {
    cerr << "Usage check_drs4_run0e_e16 run [subrun] [maxevent]" << endl;
    exit(-1);
  }
  ...
  check_drs4_run0e_e16(run, subrun, basedir);
}
double check_drs4_run0e_e16::call(vector<vector<double> > *vec, Int_t arg1, Int_t arg2){
...
void check_drs4_run0e_e16::check(Int_t run, Int_t subrun){
  ...
  for (Int_t itdc = 0; itdc < ntdc; itdc++){
	...
	h_lw1[itdc][ch] = new TH2D(Form("h_lw1_%d_%d", itdc, ch), "leading vs width;(ns)", 800, -100., 100.,600,0.,150.);
        ...
        h_le1[itdc][ch] -> SetLineColor(kRed);
        h_le2[itdc][ch] -> SetLineColor(kBlue);
        h_le3[itdc][ch] -> SetLineColor(kBlack);
        h_tr1[itdc][ch] -> SetLineColor(kRed);
        h_tr2[itdc][ch] -> SetLineColor(kBlue);
        h_tr3[itdc][ch] -> SetLineColor(kBlack);
        h_wi1[itdc][ch] -> SetLineColor(kRed);
        h_wi2[itdc][ch] -> SetLineColor(kBlue);
        h_wi3[itdc][ch] -> SetLineColor(kBlack);

        h_lw1[itdc][ch] -> SetMarkerColor(kRed);
        h_lw2[itdc][ch] -> SetMarkerColor(kBlue);
        h_lw3[itdc][ch] -> SetMarkerColor(kBlack);

        h_lt1[itdc][ch] -> SetMarkerColor(kRed);
        h_lt2[itdc][ch] -> SetMarkerColor(kBlue);
        h_lt3[itdc][ch] -> SetMarkerColor(kBlack);
  ...
  TFile *fout = new TFile(Form("ana/check_drs4_run0e_e16/check_drs4_run0e_run%s.root", runtag),"RECREATE");
...
#ifdef E16DAQ    
    Int_t spillID_prev = 0;
    Int_t event_number_prev = 0;
#endif    
  for(Int_t event = 0; event < nentries; event++){
...
#ifdef E16DAQ
      if (debug1) cout << "event: event_number, spillID = " << event << " " << event_number << " " << spillID << endl;
      Double_t delta_event_number = event_number - event_number_prev;
      if (delta_event_number < 0) {
	delta_event_number += 8;//3-bit number cycling
      }
      if (debug1) cout << "event_number, event_number_prev, delta_event_number = " << event_number << " " << event_number_prev << " " << delta_event_number << endl;
      Double_t delta_spillID = spillID - spillID_prev;
      if (debug1) cout << "spillID, spillID_prev, delta_spillID = " << spillID << " " << spillID_prev << " " << delta_spillID << endl;
      if (event>0) {
	if ((delta_event_number>1)||(delta_spillID!=0)) {
	  if ((delta_event_number>1)&&(delta_spillID!=0)) {
	    cout << "Found spill boundary" << endl;
	    spillID_tag ++;
	    cout << "spillID, event_number, spillID_tag spillID_prev, event_number_prev = " << spillID << " " << event_number << " " << spillID_tag << " "<<spillID_prev << " " << event_number_prev << endl;
	    
	    if (delta_event_number != 4) {
	      cerr << "inconsistent spill boundary with delta_event !=4 " << endl;
	      cout << "spillID, event_number, spillID_prev, event_number_prev = " << spillID << " " << event_number << " "<<spillID_prev << " " << event_number_prev << endl;
	      event_tag += delta_event_number;
	      //   exit(-1);
	    } else {
	      event_tag += delta_event_number;
	    }
	  } else {
	    cerr << "wrong spill boundary" << endl;
	    cout << "spillID, event_number, spillID_prev, event_number_prev = " << spillID << " " << event_number << " "<<spillID_prev << " " << event_number_prev << endl;
	    //	  exit(-1);
	    event_tag += delta_event_number;
	  }
	  ...
	  event_tag += delta_event_number;
	  //	  cout<< "event_tag incremented = " << event_tag << endl;
	}
      } else {
	//do nothing
	event_tag = event_number;
      }
      if (debug2) cout << "final: event_number, event_tag = " << event_number << " " << event_tag << endl;
      if (debug2) cout << "final: spillID, spillID_tag= " << spillID << " " << spillID_tag << endl;
#endif      
	...
	c1->cd(ch%16+1);
	//def
	h_le1[itdc][ch] -> SetAxisRange(0,3000,"X");
	h_le2[itdc][ch] -> SetAxisRange(0,3000,"X");
	h_le3[itdc][ch] -> SetAxisRange(0,3000,"X");
	h_le1[itdc][ch] -> Draw();
	h_le2[itdc][ch] -> Draw("same");
	h_le3[itdc][ch] -> Draw("same");
	if ((ch%16+1)==16) {
	  c1->Update();
	  c1->SaveAs(Form("ana/monitor/leading%d_run%s.gif",itdc*4+ch/16,runtag));
	}
      }
    }
  ...
  hlt_AC[0]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC_run%s.gif",runtag));
  c2->Update();
  hlt_AC[1]->SetAxisRange(0,4000);
  hlt_AC[1]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC1_run%s.gif",runtag));
  c2->Update();
  hlt_AC[2]->SetAxisRange(0,4000);
  hlt_AC[2]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC2_run%s.gif",runtag));
  c2->Update();
  hlt_AC[3]->SetAxisRange(0,4000);
  hlt_AC[3]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC3_run%s.gif",runtag));
  c2->Update();
  hlt_AC[4]->SetAxisRange(0,4000);
  hlt_AC[4]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC4_run%s.gif",runtag));
  c2->Update();

  hlt_AC[5]->SetAxisRange(0,4000);
  hlt_AC[5]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC5_run%s.gif",runtag));
  c2->Update();

  hlt_AC[6]->SetAxisRange(0,4000);
  hlt_AC[6]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC6_run%s.gif",runtag));
  c2->Update();

  hlt_AC[7]->SetAxisRange(0,4000);
  hlt_AC[7]->Draw();
  c2->SaveAs(Form("ana/monitor/hlt_AC7_run%s.gif",runtag));
  c2->Update();

  hnh_AC[0]->Draw();
  c2->SaveAs(Form("ana/monitor/hnh_AC_run%s.gif",runtag));
  c2->Update();

  hwt_AC[0]->Draw();
  c2->SaveAs(Form("ana/monitor/hwt_AC_run%s.gif",runtag));
  c2->Update();

  hlt_wt_AC[0]->SetAxisRange(0,4000,"Y");
  hlt_wt_AC[0]->Draw("colz");
  c2->SaveAs(Form("ana/monitor/hlt_wt_AC_run%s.gif",runtag));
  c2->Update();

  hadc_AC[0]->SetAxisRange(0,maxad);
  hadc_AC[0]->Draw();
  c2->SaveAs(Form("ana/monitor/hadc_AC_run%s.gif",runtag));
  c2->Update();

  hamp_AC[0]->SetAxisRange(0,maxam);
  hamp_AC[0]->Draw();
  c2->SaveAs(Form("ana/monitor/hamp_AC_run%s.gif",runtag));
  c2->Update();

  hadc_wt_AC[0]->SetAxisRange(0,40,"X");
  hadc_wt_AC[0]->SetAxisRange(0,maxad,"Y");
  hadc_wt_AC[0]->Draw("colz");
  c2->SaveAs(Form("ana/monitor/hadc_wt_AC_run%s.gif",runtag));
  c2->Update();

  hamp_wt_AC[0]->SetAxisRange(0,40,"X");
  hamp_wt_AC[0]->SetAxisRange(0,maxam,"Y");
  hamp_wt_AC[0]->Draw("colz");
  c2->SaveAs(Form("ana/monitor/hamp_wt_AC_run%s.gif",runtag));
  c2->Update();
...
check_drs4_run0e_e16::check_drs4_run0e_e16(Int_t run, Int_t subrun, TString basedir) {
  base = basedir;
