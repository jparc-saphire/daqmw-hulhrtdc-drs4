...
void timewalkcorr1()
{
  ...
  for(int i = 0; i<n; ++i){
    ...
    if(178.0<tdc1[0]&&tdc1[0]<187.0&&178.0<tdc2[0]&&tdc2[0]<187.){
      ...
      double w1 = width1[0]-3.8;
      double w2 = width2[0]-4.2;
      
      hist[0]->Fill(w1,(tdc1[0]-tdc2[0])-0.50);
      hist[0]->GetXaxis()->SetTitle("WIDTH1");
      hist[0]->GetYaxis()->SetTitle("TOF");
      
      hist[1]->Fill(w2,(tdc1[0]-tdc2[0])-0.50);
  ...
  for(int i = 0; i<NofProject1; ++i){
    ...
    tmp1->Fit("fit1", "Q", "", center1-1.0, center1+1.0);
  ...
  for(int i = 0; i<NofProject2; ++i){
    ...
    double center1 = tmp1->GetBinCenter(tmp1->GetMaximumBin());
    tmp1->Fit("fit2", "Q", "", center1-1.0, center1+1.0);
