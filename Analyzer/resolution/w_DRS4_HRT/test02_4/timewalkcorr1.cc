...
void timewalkcorr1()
{
  TFile *fin = new TFile("../../test_run025.root");
  ...
  TH2F *hist[2];
  hist[0] = new TH2F("h1", "TOF1vsPE1", 1000, 0, 30,  80, -3, 3);
  hist[1] = new TH2F("h2", "TOF1vsPE2", 1000, 0, 30,  80, -3, 3);
  ...
  for(int i = 0; i<n; ++i){
    ...
    if( tdc1[0]>0.&&tdc2[0]>0.&&amp1[0]>0.&&amp2[0]>0.){
      ...
      double pe1 = 0.0830*amp1[0]+2.24;
      double pe2 = 0.0847*amp2[0]+2.47;

      double Amp1 = pe1-4.5;
      double Amp2 = pe2-4.5;
      
      if( Amp1>2 && Amp2>2 ){
	hist[0]->Fill(Amp1,(tdc1[0]-tdc2[0])-0.53);
	hist[0]->GetXaxis()->SetTitle("PE1");
	hist[0]->GetYaxis()->SetTitle("TOF1");
	
	hist[1]->Fill(Amp2,(tdc1[0]-tdc2[0])-0.53);
