...
void timewalkcorr1()
{
  ...
  TH2F *hist[2];
  hist[0] = new TH2F("h1", "TOF1vsPE1", 1000, 0, 30, 50, -3, 3);
  hist[1] = new TH2F("h2", "TOF1vsPE2", 1000, 0, 30, 50, -3, 3);
  ...
  for(int i = 0; i<n; ++i){
    ...
    if( ltdc1[0]>0.&&ltdc2[0]>0.&&amp1[0]>0.&&amp2[0]>0.){
      ...
      if( Amp1>0 && Amp2>0 ){
	hist[0]->Fill(Amp1,(ltdc1[0]-ltdc2[0])+0.70);
	hist[0]->GetXaxis()->SetTitle("PE1");
	hist[0]->GetYaxis()->SetTitle("TOF1");
	
	hist[1]->Fill(Amp2,(ltdc1[0]-ltdc2[0])+0.70);
