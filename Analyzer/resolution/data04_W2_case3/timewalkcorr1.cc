...
void timewalkcorr1()
{
  ...
  TH2F *hist[2];
  hist[0] = new TH2F("h1", "TOF1vsWIDTH1", 1000, 0, 12, 50, -3, 3);
  hist[1] = new TH2F("h2", "TOF1vsWIDTH2", 1000, 0, 12, 50, -3, 3);
  ...
  for(int i = 0; i<n; ++i){
    ...
    if(172.0<ltdc1[0]&&ltdc1[0]<182.0&&172.0<ltdc2[0]&&ltdc2[0]<182.0&& 
      ...
      if( w1>0.0 && w2>0.0 && pe1>0.0 && pe2>0.0){
	hist[0]->Fill(w1,(ltdc1[0]-ltdc2[0])+0.22);
	hist[0]->GetXaxis()->SetTitle("WIDTH1");
	hist[0]->GetYaxis()->SetTitle("TOF");
	
	hist[1]->Fill(w2,(ltdc1[0]-ltdc2[0])+0.22);
